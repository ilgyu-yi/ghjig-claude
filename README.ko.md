...
**실행 방법 — `ghjig` 또는 그냥 `claude`.** Install에서 만든 `ghjig` PATH 래퍼는 어디서든 동작합니다. 등록된 target 안에서는 그냥 `claude`로 띄워도 됩니다: hook이 `.claude/ghjig-root` 바인딩 심링크로 셸을 스스로 찾아가므로 전역 `GHJIG_SHELL_ROOT` env가 필요 없습니다(SPEC §3.2.1).
