...
- `directive-protect` — `git checkout -b <user>/<type>/<N>-<slug>` blocked because `<N>` is a Directive Issue (SPEC §1.7 / §6.1 type-aware hook). Directives are gated by `/complete-directive` + `directive-reviewer`, not engineering-flow `Closes #N` semantics. First-line remedy: `/activate-directive <N>` (if Proposed) or `/file-issue --parent <N>` then `/work-on` the new Issue (if Active). Escape is for cases where the maintainer explicitly wants to repurpose the Directive Issue as an Execution branch — rare and audit-logged.
