...
3.5. **Finding judgment — dispatch `finding-judge` (SPEC §4.13, #707), BEFORE step 4 applies any verdict.** When the step-3 report **enumerates findings**, dispatch a **fresh, worktree-isolated `finding-judge` invocation per round, with no shared context**: pass the Issue number and the `activation-reviewer` report **verbatim** (what the reviewer actually wrote, never a paraphrase), and record `dispatch:` — which invocation produced the findings and which produced the judgment, since the judge cannot self-observe that limb. **Never hand-carry a prior round's judged list into the prompt** — the judge fetches it itself (`scripts/ghjig_judged_list.sh rounds --issue <N>`). A report with **zero findings pays no dispatch** — go straight to step 4. Post the judged list durably on the Issue via `scripts/ghjig_judged_list.sh post --issue <N> <judge-output-file>` **before** step 4 applies any verdict. If the judge is unavailable, returns malformed output, **or the post fails** (transient `gh`), proceed on the raw findings, record `judgment: unavailable` (or `durable: post-failed`), emit an `audit_log warn` — **never park**. The one exception is a **stale-pin refusal**: the body advanced under the judgment, so the raw findings are stale too — re-dispatch the judge at the current body instead of proceeding (SPEC §4.13). The reviewer's `VERDICT:` line is untouched by judgment (the step-4 matrix keys on it exactly as before). Do not restate the judge's output schema here — `.claude/agents/finding-judge.md` is the contract SSOT (§9).
