...
- **Not from `/ship` automatically** — auto-posting the comment from `/ship`'s procedure would couple two distinct artifacts (the ship action + the dir-mode reflection) and make idempotency harder to reason about. `/ship` post-merge step records the bare `directive-exec-count` audit; `/reflect` is the human-readable counterpart; the workflow is the hook-enforced floor.
