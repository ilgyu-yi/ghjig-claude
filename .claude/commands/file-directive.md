...
1.7. **Citation check** (SPEC §1.10, #676) — resolve the body's quoted spans before the gate step below, so a site-mismatched or fabricated quotation is corrected by the author rather than found by the reviewer:
   ...
   - Then `rm -f .claude/ghjig-state/tmp/proposed-directive-body.md`. The staged file is a full draft of a body that has not passed its reviewer gate, nothing downstream reads it again, and leaving it behind accumulates pre-review drafts on disk indefinitely.
