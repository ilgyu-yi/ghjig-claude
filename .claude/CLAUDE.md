...
- **Per-project ephemeral state** (SPEC §3.2.2; #314, Directive #311): the audit log and `.claude/state` caches resolve via `eng_state_dir()` (in `hookrt.sh`) — `ENG_STATE_DIR_OVERRIDE` → `$CLAUDE_PROJECT_DIR/.claude/eng-state` when `CLAUDE_PROJECT_DIR` is set (hook context) → else **empty**, and callers fall back to the legacy shared `$CLAUDE_ENG_SHELL_ROOT/.claude/{audit,state}`. So in hook context two projects' audit/caches are mutually invisible; outside it (plain Bash, env unset) behavior is unchanged. The scope-guard **registry** is NOT yet per-project (deferred to a follow-up EI); `eng-state/` is added to the target's `.git/info/exclude`.
