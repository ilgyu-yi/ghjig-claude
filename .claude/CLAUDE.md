...
- **Audit observability** (SPEC §6.1, Directive #356 / #361): audit records carry an additive `source` field (`live` default; `test` only via the harness-owned `CLAUDE_ENG_AUDIT_SOURCE=test` marker a real action cannot inject — anti-reclassification, §7), and the three reviewers emit a categorized reject record (`reviewer_reject_audit` → `warn <issue-review|plan-review|activation> reject "class=<token> …"`) on a non-pass verdict. Both are observability surfaces, not gates — full contract in SPEC §6.1.
