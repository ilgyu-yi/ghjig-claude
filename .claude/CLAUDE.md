...
Escape — two audit-logged forms: a trailing `# claude-eng:skip=<cat> reason=<why>` sentinel and a leading `SKIP_HOOKS=<cat> SKIP_REASON='<why>'` env-prefix (leading wins if both present). **Observed reality (this harness):** both are **stripped before the hook in the live Claude Code Bash tool** — the parsers are correct but the harness never delivers the prefix/sentinel to `tool_input.command`, so neither takes effect in-harness. They work only where the command arrives verbatim (a real shell, the smoke harness). For a sanctioned in-agent guarded op (e.g. the `/release` branch commit), run it in a real terminal or via a non-protected branch + rename; #479 tracks restoring an in-agent channel. Full contract: SPEC §7.
