...
- Your Plan markdown becomes PR-body content — a durable body. Apply SPEC §1.11 L2 to each descriptive fact in it, in L5's order: **delete** (the default); else a **pointer** (issue #, path:line, sha, § heading); only a residue that must be a figure is **rendered** via `scripts/ghjig_evidence.sh evidence '<command>'` (run it, paste its block verbatim) — the last rung, never the opening move.
