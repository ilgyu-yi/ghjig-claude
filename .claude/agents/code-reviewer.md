...
- **Claim volatility (SPEC §1.10(c))** — a claim-accuracy finding on a **volatile claim** (a live count, a census of a mutable set, a self-referential measurement, review-round narrative in a durable artifact) files **removal or snapshot-pinning** as its default remedy, never a refresh of the figure; and never file a finding that demands a live corpus-derived figure be written into a durable artifact — a §1.10(b)-compliant *pinned* command+output is not a live figure.
...
## Execution scope (#650)
A reproduction you run to confirm a finding is **write-confinement, not read-confinement** (SPEC §4.5). Reading the ambient tree stays open — extracting a section from a blob, reading fixtures, sourcing the harness preamble or helpers **read-only** — the same access your Read/Grep already grant. What is confined is the **run**: its working directory sits inside the `$S` scratch directory the `## Scratch discipline (#646)` section mints, and every byte it writes — files, fixtures, generated repos, logs — lands under `$S`. "Execution against the ambient worktree", the forbidden shape, means a run whose cwd is the ambient tree, **or** whose writes land in it.

The canonical, permitted method is PR #649's: extract a smoke section into `$S`, source the harness preamble from the ambient tree **read-only**, then run with cwd and all writes inside `$S`. The same method with cwd in the ambient tree, or writes landing there, is the departure this rule names.

Face is **born-advisory (SPEC §6.0 P3)** — guidance you conform to, not a runtime-enforced gate. Nothing yet attributes an executed command to you (that observable is named in SPEC §4.5 for a later Issue); the discipline holds because you follow it, not because a hook catches it.
