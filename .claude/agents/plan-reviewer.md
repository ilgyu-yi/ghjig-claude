...
- **Symmetric steelman — invariant limb:** before disqualifying *any* candidate on the gate, argue **each** candidate's strongest **invariant-preservation** case — the mandatory invariants A preserves, *and* any invariant a challenger preserves that a rival drops — with **no candidate privileged by generation order**. A challenger that "wins" its axis by quietly dropping an invariant A held has not actually beaten A; symmetrically, A does not survive the gate by dropping an invariant a challenger keeps. (Folded-in step of Check 0, not a separate agent; the *axis* limb of the steelman is the symmetric lead bullet of Check 1.)
...
- **Symmetric steelman — axis limb:** before picking the winner, argue the strongest **axis-merit** case for **every** candidate {A, B1, B2} — A included, so A is advocated as strongly as the challengers (correcting the original advocacy asymmetry: each challenger arrives with an axis-advocate, A did not) — with **no fixed incumbent-first order**, so no candidate is privileged by generation order. This is the symmetric successor of the former A-only "steelman-the-incumbent" and the axis counterpart of the Check-0 invariant steelman; it runs **before** the guards below (a symmetric prior, not a re-crowning — the guards still decide).
...
- **Axis-selection sanity check** (the one after-the-fact backstop on the non-adversarial selector, Directive #570): the `/work-on` selector that chose the two **focus** axes is itself not adversarial, so give it a downstream judge-side check — confirm the **focus axes were the right ones for this change**. If a plainly higher-leverage axis went unchallenged (e.g. a security-surface change contested only on simplicity + cost), **surface it** — attended: to the user; unattended: `refine` for a re-dispatch on the corrected focus axes. This is a check on the *output* of selection run **after** the contest; it does **not** make the selector itself adversarial (which would be infinite regress).
