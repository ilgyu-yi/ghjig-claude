...
- **MISSION.md alignment**: the `MISSION fit` section must name a specific `MISSION.md` section or success criterion. If the target repo has no `MISSION.md`, note that in the verdict reason and pass through to manual review — the Directive may motivate a MISSION.md amendment, which is appropriate. The legacy Goal-bootstrap allowance is **removed** for repos that have a `MISSION.md`; for repos onboarding the shell whose first Directive precedes their first `MISSION.md`, the allowance still applies (note the absence + pass through).
